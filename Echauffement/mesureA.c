...
int main() 
{
    ...
    FILE *pot = fopen("mesureDeA_sans_equilibre2.txt", "w");
    double A = 0;
    int nstep = 5000;
    ...
    {
        A += Metropolis(tab_particules_lamda_i, N, T0, L);
    ...
    for (int i = 2; i <=1000; i++)
    {
        ...
        if (i%10==0)
        {
            for (int step = 0; step < nstep; step++)
            {
                A += Metropolis(tab_particules_lamda_i, N, T, L);
                conditions_periodiques(tab_particules_lamda_i, N, L);
            }
            fprintf(pot, "%g %g\n",  T,A/nstep);
            // for (int i = 0; i< N; i++)
    ...
    system("gnuplot -persist -e \"plot 'mesureDeA_sans_equilibre2.txt' with lines title 'Potentiel'\"");
    //mesureDeA2 sans copie de l'equilibre
